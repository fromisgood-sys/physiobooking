...
> **Local install note (not part of the upstream skill):** installed from
> https://github.com/nextlevelbuilder/ui-ux-pro-max-skill with only `SKILL.md`
> and `references/*.md` — the `scripts/` (Python) and `data/` (~2.5MB of
> CSV/JSON) directories were deliberately left out, so every `python
> "${CLAUDE_PLUGIN_ROOT}/.claude/skills/ui-ux-pro-max/scripts/search.py" ...`
> command below **will fail** (no such file). Use the priority table and the
> two reference files directly instead — they carry the actual rule content
> and don't need the search tool. For this project specifically, DESIGN.md
> at the repo root is the binding design contract; treat everything below as
> supplementary general guidance, not a replacement for it.
...
**Not installed in this project** — see the note at the top of this file. The commands below are upstream documentation, kept for reference in case `scripts/` and `data/` are added later.
